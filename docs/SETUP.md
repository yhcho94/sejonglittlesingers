...
2. `supabase/migrations/` 폴더의 파일을 **번호 순서대로** 하나씩, 내용 **전체**를 붙여넣고 **Run**
   ...
   - `0022_staff_roles_parent_reps.sql`: 운영진 역할(가입 후 마이페이지에서 수정) · 반별 학부모 대표 지정(최상위 관리자)
   - `0023_org_chart.sql`: 운영진 담당 반·조직도 게시, 학부모 부대표, 회원 구분 변경, 조직도 표시 방식(예전 고정 ↔ 회원 정보로 자동)
