...
export function SignupForm() {
  ...
  const [type, setType] = useState<MemberType>("parent");
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      {!parent && (
        <>
          <div>
            <label htmlFor="affiliation" className="label">{type === "teacher" ? "담당" : "소속·역할"} *</label>
            <input
              id="affiliation"
              name="affiliation"
              required
              maxLength={100}
              placeholder={type === "teacher" ? "예: 지휘, 반주, 화음반 담당" : "예: 운영위원회 홍보 담당"}
              className="input"
            />
          </div>
          <div>
      ...
      <div className="space-y-3 rounded-sm bg-cream p-4 text-sm">
        ...
        <ul className="list-disc space-y-1 pl-5 text-ink-soft">
          <li>
            수집 항목: {parent ? "보호자 이름, 연락처, 이메일" : `이름, 연락처, 이메일, ${type === "teacher" ? "담당" : "소속·역할"}, 신청 사유(선택)`}
          </li>
        ...
        {parent ? (
          ...
          <label className="flex items-start gap-2">
            ...
            <span>본인은 만 14세 이상이며, 합창단 {type === "teacher" ? "선생님" : "운영 담당자"} 본인입니다.</span>
          </label>
        )}
      </div>
