...
// 사진 크게 보기 (키보드 ←/→/Esc, 휴대폰 좌우 밀기). open 이 null 이면 닫힘
export function Lightbox({
  ...
  const move = useCallback(
    (d: number) => setOpen(open === null ? null : (open + d + photos.length) % photos.length),
    ...
    >
      ...
      >
        ...
        <img
          ...
          className="animate-rise max-h-full max-w-full object-contain"
        />
        {photos.length > 1 && (
          <>
            <button
              ...
              onClick={() => move(-1)}
              aria-label="이전 사진"
              className="absolute left-1 hidden p-3 text-white/70 hover:text-white md:block"
            >
              <svg
                viewBox="0 0 24 24"
                className="h-8 w-8"
            ...
            <button
              ...
              onClick={() => move(1)}
              aria-label="다음 사진"
              className="absolute right-1 hidden p-3 text-white/70 hover:text-white md:block"
            >
              <svg
                viewBox="0 0 24 24"
                className="h-8 w-8"
